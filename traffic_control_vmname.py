...
class TrafficControl(object):
    ...
    def save_filter(self, filters):
        try:
            ...
            for ipaddress in filters:
                ...
                filter_rules[ipaddress]['vmname'] = filter_rule.vmname
    ...
    def resize_filter(self, vmname, rate):
        self.logger.debug('delete filter vmname: %s rate' % (vmname))
        ipaddress = self.get_vm_ipaddress(vmname)
        if not ipaddress:
            return
        self.logger.debug('delete filter ipaddress: %s rate' % (ipaddress))
        filter_rule = self.filters[ipaddress]
        self.add_filter(vmname, ipaddress, filter_rule.ifname,
            filter_rule.rate)
        return

    def add_filter(self, vmname, ipaddress, ifname, rate, save=True):
        ...
        flowid = "1:" + str(flowid)
        self.filters[ipaddress] = FilterRule(vmname, ifname, rate, flowid)
    ...
    def get_vm_ipaddress(self, vmname):
        for ipaddress in self.filters:
            filter_rule = self.filters[ipaddress]
            if vmname == filter_rule.vmname:
                return ipaddress
                break
        return

    def delete_filter(self, vmname, save=True):
        '''
    ...
    def show_filter(self, ipaddress):
        ...
        if ipaddress in self.filters:
            filter_rule = self.filters[ipaddress]
            vmname = filter_rule.vmname
            ifname = filter_rule.ifname
            flowid = filter_rule.flowid
            rate = filter_rule.rate
            print('vmname: %s, ifname: %s, ip: %s, priority: %s, rate: %s, flowid: %s'
                % (vmname, ifname, ipaddress, self.priority, rate, flowid))
        else:
    ...
    def reload(self):
        ...
        for ipaddress in self.filters:
            ...
            rate = filter_rule.rate
            vmname = filter_rule.vmname
            self.add_filter(vmname, ipaddress, ifname, rate, save=False)
        return
...
class FilterRule(object):
    ...
    def __init__(self, vmname, ifname, rate, flowid):
        self.vmname = vmname
        self.ifname = ifname
        self.rate = rate
        self.flowid = flowid
...
def controller():
    ...
    usage = """usage: %prog [options]
        -a -n ifname -v vmname -i ipaddress -r rate
        -d -i ipaddress
        -s -i ipaddress"""
    ...
    parser.add_option('-v', '--vmname', action='store', type="string",
        dest='vmname', help='virtual server name')
    parser.set_defaults(rate=1)
    (options, args) = parser.parse_args()
    traffic_control = TrafficControl()
    if (options.add and options.vmname and options.ipaddress and \
        options.rate and options.ifname):
        rate = str(int(options.rate)) + "Mbit"
        traffic_control.add_filter(options.vmname, options.ipaddress, \
            options.ifname, rate)
    elif options.delete and options.vmname:
        traffic_control.delete_filter(options.vmname)
